...
LOGS_DIR="${BASE_DIR}logs/multigets_non_sharded/${TIMESTAMP}/"
...
for ratio in ${RATIOS[@]}; do
  ...
  for run in `seq 1 ${REPETITIONS}`; do
    ...
    for vclients in ${VCLIENTS[@]}; do
        ...
        LOG_NAME="${LOGS_DIR}ratio${ratio}_run${run}_vclients${vclients}_workerthreads${workerthreads}"
        ...
        ADD="_ratio${ratio}_run${run}_vclients${vclients}_workerthreads${workerthreads}.log"
